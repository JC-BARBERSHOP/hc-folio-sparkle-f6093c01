...
function Index() {
  ...
  const LangToggle = ({ className = "" }: { className?: string }) => (
    <div className={`inline-flex items-center rounded-full border border-foreground/15 bg-background/60 p-1 text-sm ${className}`}>
      <button onClick={() => changeLang("en")} aria-pressed={lang === "en"} className={`rounded-full px-2.5 py-1 font-medium transition-colors sm:px-3 sm:py-1.5 ${lang === "en" ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"}`}>EN</button>
      <button onClick={() => changeLang("es")} aria-pressed={lang === "es"} className={`rounded-full px-2.5 py-1 font-medium transition-colors sm:px-3 sm:py-1.5 ${lang === "es" ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"}`}>ES</button>
    </div>
  );
  return (
